...
namespace TS_AutomationStudio.Test_Cases_PROD_Environment
{
    class ExportDataAPI
    {
        ...
        static string WebServiceAPI = APICollection.exportData.ToString();
        ...
        public void ExportTR(Logger loggerObj, string[] OptionalParams)
        {
            Console.WriteLine("\n" + DateTime.Now + " : " + "|> ExportTR");

            ExportDataAPI.loggerObj = loggerObj;
            ...
            try
            {
                if (StudioHome.isPerformanceSuiteRunning)
                {
                    ...
                    filePaths = Directory.GetFiles(TS_AutomationStudio.Resources.ExportData_TR.InputFilesLocation + userNo, "*.xml");
                ...
                {
                    filePaths = Directory.GetFiles(TS_AutomationStudio.Resources.ExportData_TR.InputFilesLocation, "*.xml");
            ...
            {
                Console.WriteLine("\nSEVERE ERROR : Input Files Location : " + TS_AutomationStudio.Resources.ExportData_TR.InputFilesLocation + " does not exist !");
                return;
            }
            ...
            foreach (string file in filePaths)
            {
                ...
                //Variables
                string UserName = "idautoma";
                string Password = "Welcom3$";
                string sessionID = string.Empty;
                int entitytype = 0;
                ...
                exportData expdObj = new exportData();
                expdObj.SessionID = SessionID;
                expdObj.strData = inputxml.InnerXml;
                expdObj.ielementType = entitytype;

                exportDataRequest edr = new exportDataRequest();
                edr.exportData = expdObj;


                init = StopWatch.Start();
                API_Invoke_Time = init;

                exportDataResponse1 edrs = null;

                try
                {
                    edrs = (exportDataResponse1)port.exportData(edr);
                }
                catch (Exception ex)
                {
                    Result = "Fail";
                ...
                WebService_Response_Dump = edrs.exportDataResponse.@return;
