...
namespace TS_AutomationStudio.Test_Cases_PROD_Environment
{
    class LoginAPI
    {
        ...
        static string WebServiceAPI = APICollection.login.ToString();
        ...
        public void Login(Logger loggerObj, string[] OptionalParams)
        {

            Console.WriteLine("\n" + DateTime.Now + " : " + "|> Login");

            LoginAPI.loggerObj = loggerObj;
            ...
            try
            {
                if (StudioHome.isPerformanceSuiteRunning)
                {
                    ...
                    filePaths = Directory.GetFiles(TS_AutomationStudio.Resources.Login.InputFilesLocation + userNo, "*.xml");
                ...
                {
                    filePaths = Directory.GetFiles(TS_AutomationStudio.Resources.Login.InputFilesLocation, "*.xml");
            ...
            {
                Console.WriteLine("\nSEVERE ERROR : Input Files Location : " + TS_AutomationStudio.Resources.Login.InputFilesLocation + " does not exist !");
                return;
            }
            ...
            foreach (string file in filePaths)
            {
                ...
                logObj.ldapID = UserName;
                logObj.ldapPasswd = Password;

                TestStudio port = new TestStudioClient();

                loginRequest lr = new loginRequest(logObj);

                init = StopWatch.Start();
                API_Invoke_Time = init;

                loginResponse1 lres = null;
                ...
                WebService_Response_Dump = lres.loginResponse.@return;
